<x-app-layout>
        ...
        <form action="" method="POST" id="editCurriculumForm">
            ...
            <div class="row ms-2 me-2">
                <div class="row mb-3">
                    ...
                    <div class="col-md-6">
                        <label for="serie" class="form-label">Série/Etapa</label>
                        <select class="form-select" name="series" id="series" value="{{ old('series') }}" required>
                            <option selected>Selecione uma série</option>
                            <option value="educ_infa_cc_0_3">Educacao Infantil - Creche (0 a 3 anos)</option>
                            <option value="educ_infa_cc_4_5">Educacao Infantil - Pre-escola (4 a 5 anos)</option>
                            <option value="educ_ini_1_5">Ensino Fundamental - Anos Iniciais (1 ao 5 ano)</option>
                    ...
                    <div class="col-md-6">
                        <label for="modality" class="form-label">Modalidade</label>
                        <select class="form-select" id="modality" name="modality" value="{{ old('modality') }}" required>
                            <option selected>Selecione uma modalidade</option>
                            <option value="bercario">Berçário</option>
                            <option value="creche">Creche</option>
                            <option value="pre_escola">Pré-escola</option>
                    ...
                    <div class="col-md-6">
                        <label for="weekly_hours" class="form-label" required>Horas semanais</label>
                        <input type="text" class="form-control" id="weekly_hours" name="weekly_hours" placeholder="Ex. 40"
                            value="{{ old('weekly_hours') }}" required>
                    </div>

                    <div class="col-md-6">
                        <label for="total_hours" class="form-label" required>Total de horas</label>
                        <input type="text" class="form-control" name="total_hours" id="total_hours" placeholder="Ex. 2000"
                            value="{{ old('total_hours') }}" required>
                    </div>

                    <div class="col-md-6">
                        <label for="start_time" class="form-label" required>Hora início</label>
                        <input type="time" class="form-control" name="start_time" id="start_time" placeholder="Ex. 40"
                            value="{{ old('start_time') }}" required>
                    </div>

                    <div class="col-md-6">
                        <label for="end_time" class="form-label" required>Hora final</label>
                        <input type="time" class="form-control" name="end_time" id="end_time" placeholder="Ex. 2000"
                            value="{{ old('end_time') }}" required>
                    </div>
    ...
    @section('scripts')
        <script>
             function showCurriculum(uuid) {
                $.ajax({
                    url: "{{ route('manage.curriculum.show', '') }}" + "/" + uuid,
                    type: "GET",
                    dataType: "json",
                    success: function(data) {
                        $('#editCurriculumForm').attr('action', "{{ route('manage.curriculum.update') }}");
                        $('#editCurriculumForm').attr('method', "POST");
                        $('#editCurriculumForm').append('<input type="hidden" name="curriculum" value="' + uuid + '">');
                                ...
                                <form action="#" method="POST">
                                    ...
                                    <div class="row">
                                        <div class="btn-group" role="group" aria-label="Basic example">
                                            <button type="button" class="btn btn-outline-primary btn-sm" data-bs-toggle="modal" data-bs-target="#editCurriculum" onclick="showCurriculum('${row.uuid}')">
                                                Editar
                                            </button>
                                            <a href="#" class="btn btn-outline-secondary btn-sm">
                                                Disciplinas
                                            </a>
                                            <button type="button" class="btn btn-outline-danger btn-sm" data-bs-toggle="modal" data-bs-target="#deleteCurriculum">
                                                Excluir
                                            </button>
                                        </div>
                                    </div>
